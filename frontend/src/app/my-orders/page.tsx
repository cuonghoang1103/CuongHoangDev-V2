...
import { getOrderByCode, type OrderResponse } from '@/lib/api/shop';
...
export default function MyOrdersPage() {
  ...
  // Fetch orders from backend using stored order codes
  useEffect(() => {
    ...
    if (storedOrders.length > 0) {
      ...
      Promise.allSettled(
        storedOrders.map((o) =>
          getOrderByCode(o.id).then((res) => res.data).catch(() => null)
        )
      ).then((results) => {
