    ...
    <div className="space-y-3">
      {socialProviders.map((provider) => (
        <motion.button
          ...
          className="w-full py-3 px-4 bg-white/5 border border-darkborder rounded-xl text-text-primary font-medium hover:bg-white/10 hover:border-white/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
        >
          {loading === provider.id ? (
...
function LoginForm() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      const dest = redirect || (isAdmin ? '/admin' : '/');
      router.push(dest);
    } catch (err: unknown) {
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  {...register('remember')}
                  type="checkbox"
                  className="w-4 h-4 rounded border-darkborder bg-darkbg text-neon-violet focus:ring-neon-violet/30 cursor-pointer accent-neon-violet"
                />
