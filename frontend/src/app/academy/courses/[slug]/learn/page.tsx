...
import {
  ...
  Download, BookOpen, Loader2, Play, ChevronDown, ChevronUp, ArrowLeft, Lock
} from 'lucide-react';
...
export default function AcademyLearnPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
      <div className="min-h-screen bg-darkbg flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/academy" className="text-neon-violet hover:text-neon-indigo">Back to Academy</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-darkbg flex flex-col">
      ...
      <header className="h-14 bg-darkcard border-b border-darkborder/50 flex items-center justify-between px-4 shrink-0 z-20">
        <div className="flex items-center gap-3">
          ...
          <Link href={`/academy/courses/${slug}`} className="text-text-muted hover:text-text-primary transition-colors">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-xs text-text-muted">{overallProgress}%</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <aside className={`${sidebarOpen ? 'w-80' : 'w-0'} bg-darkcard border-r border-darkborder/50 overflow-y-auto transition-all shrink-0`}>
          <div className="p-4">
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-2">
                      ...
                      <p className="text-xs text-text-muted mt-0.5">
                        {section.lessonCount} lessons &bull; {section.lessons?.filter(l => isCompleted(l.id)).length || 0}/{section.lessonCount}
                      </p>
                    </div>
              ...
              {currentLesson.documents && currentLesson.documents.length > 0 && (
                <div className="bg-darkcard border border-darkborder/50 rounded-2xl p-6 mb-8">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-semibold text-text-primary">Materials</h3>
                  </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-text-muted">
                            {doc.fileType?.toUpperCase()} &bull; {doc.downloadCount} downloads
                          </p>
                        </div>
                      </a>
                    ))}
              ...
              <div className="flex items-center justify-between pt-6 border-t border-darkborder/30">
                ...
                {nextLesson ? (
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 bg-green-500/10 text-green-400 font-medium rounded-xl hover:bg-green-500/20 transition-all text-sm"
                  >
                    <CheckCircle className="w-4 h-4" />
                    Course Completed
                  </Link>
